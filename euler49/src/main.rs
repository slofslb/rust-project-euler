...
fn main() {
    ...
    for x in 1000..9999 {//9999 {
        if !prime_mask[x] {continue;} 
        let mut vx :Vec<u32> = x.to_string().chars().map(|c| c.to_digit(10).unwrap()).collect();
        //println!("{:?}", vx);

        let mut ys = Vec::new();
        heap_recursive(&mut vx, |pt| {
            let perm_x = pt.to_vec().iter().map(|p| p.to_string()).collect::<String>().parse::<u32>().unwrap();
            ys.push(perm_x)
        });
        //println!("{:?}", ys);
        ...
        for y in ys.iter() {
            ...
            if diff <= 0 {continue};
            if !prime_mask[*y as usize] {continue;}
            let z = diff as u32 + y;
